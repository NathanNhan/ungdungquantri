...
const deleteMobile = (key) => {
    if (confirm('Hệ thống sẽ Xóa Dữ liệu...?')) {
        ...
        apiDienthoaiDelete(condition, user.access_token).then(result => {
...
const saveMobile = () => {
    ...
    if (capNhat) {
        ...
        //return false;
        // Call API
        apiDienthoaiInsert(mobileNew, user.access_token).then(result => {
            console.log(result);
            saveImg(Ma_so);
        ...
        // Call API
        apiDienthoaiUpdate(mobileUpdate, user.access_token).then(result => {
            //console.log(result);
            saveImg(Ma_so);
            apiDienthoai().then(result => {
                dsMobile = result;
